...
function closeModal() {
    ...
    app.setGeneratedImageFile(null);
}
...
function selectColorAndClose(color) {
    const targetTile = app.pointerState.longPressTarget;
    ...
    if (targetTile) {
        app.performAction(() => {
            const targetIndices = app.getSymmetricIndices(app.tiles().indexOf(targetTile));
            app.applyActionToTiles(targetIndices, tile => {
                app.clearGoldOverlay(tile);
                app.setIndex(tile, app.getSelectedColorIndex());
            });
        });
    }
...
function openColorPickerModal(targetTile = null) {
    if (app.isBreathing()) return;
    app.pointerState.longPressTarget = targetTile;
    app.setColorPickerPage(0);
    renderColorPickerContent();
...
export function initializeModals(appContext) {
    ...
    app.dom.btnNextPalette.addEventListener('click', () => app.navigateColorPages(true));
    app.dom.btnPrevPalette.addEventListener('click', () => app.navigateColorPages(false));
    app.dom.colorPickerModal.addEventListener('wheel', handlePaletteWheel);
    app.dom.colorPickerModal.addEventListener('touchstart', app.handleTouchStart, { passive: true });
    app.dom.colorPickerModal.addEventListener('touchend', app.handleTouchEnd, { passive: true });
