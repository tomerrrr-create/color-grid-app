(function() {
      ...
      const translations = {
        ...
        tooltip_gameOfLife: { en: "Run 'Life' Generation. Long-press to prepare.", he: "הפעל דור 'חיים'. לחיצה ארוכה להכנה.", ru: "Запустить поколение 'Жизни'. Долгое нажатие для подготовки.", nl: "Start 'Leven' Generatie. Lang indrukken om voor te bereiden." },
      ...
      const PALETTES = [
        { name: translations.paletteName_journey[currentLang], originalName: "Journey", emoji: '🌓', colors: ["#000000", "#424242", "#7B1FA2", "#5E35B1", "#3949AB", "#673AB7", "#8E24AA", "#AD1457", "#3F51B5", "#B71C1C", "#9C27B0", "#C2185B", "#C62828", "#2E7D32", "#D32F2F", "#D81B60", "#AB47BC", "#757575", "#1976D2", "#E91E63", "#E53935", "#FF1744", "#EC407A", "#F44336", "#1E88E5", "#0097A7", "#EF5350", "#F4511E", "#FF4081", "#43A047", "#FF5252", "#FF5722", "#2196F3", "#F06292", "#4CAF50", "#FF7043", "#F57C00", "#9E9E9E", "#42A5F5", "#FB8C00", "#66BB6A", "#FF8C00", "#00BCD4", "#AFB42B", "#F48FB1", "#FF9800", "#8BC34A", "#26C6DA", "#FFA726", "#BDBDBD", "#9CCC65", "#FBC02D", "#CDDC39", "#FFE082", "#FFEE58", "#FFF176", "#FFD700", "#FDD835", "#F5F5F5", "#FFFFFF"] },
      ...
      function toggleSimMode() {
        ...
        // MODIFIED: Toggle the class on the main container
        controlsContainer.classList.toggle('sim-mode-active', isSimModeActive);

        if (isSimModeActive) {
            // On entering the mode, perform all setup actions as a single, undoable step.
            animateBoardTransition(() => {
                performAction(() => {
                    ...
                    separatorPx = 0; applySeparator();
                    symmetryMode = 'kaleidoscope'; updateSymmetryUI();
            ...
            // ---- START: When exiting sim mode, ensure life is paused ----
            pauseLife();
            // ---- END: Ensure life is paused ----
        }

        const controlsToHide = [
            btnBrushMode, btnResizeUp, btnResizeDown, btnGap, btnResetBoard,
            btnTutorial, btnSave, btnColorPicker
        ];
    ...
    function updateColorPickerButtonUI() {
      ...
      function runGameOfLifeGeneration() {
          ...
          for (let i = 0; i < n; i++) {
              ...
              for (let j = 0; j < n; j++) {
                  ...
                  const isAlive = tileIndex !== darkestIndex && !isGold(tile);
                  currentState[i][j] = {
                      isAlive,
                      k: tileIndex,
                  };
          ...
          for (let i = 0; i < n; i++) {
              ...
              for (let j = 0; j < n; j++) {
                  ...
                  if (cell.isAlive) { 
                      becomesAlive = liveNeighbors >= 2 && liveNeighbors <= 4;
                  } else {
                      becomesAlive = liveNeighbors === 3;
                      if (becomesAlive) {
                          const geneticColor = getGeneticColor(neighborColors);
      ...
      function runBrightnessEvolution() {
          ...
          for (let i = 0; i < n * n; i++) {
              ...
              if (cell.isGold) {
                  nextStateK[i] = cell.k; // תאי זהב לא משתתפים בחישוב
                  continue;
              }
              ...
              // לולאה שעוברת על התא וכל 8 שכניו (רשת של 3x3)
              for (let di = -1; di <= 1; di++) {
                  for (let dj = -1; dj <= 1; dj++) {
                      const ni = row + di;
                      const nj = col + dj;

                      // בדיקה שהשכן נמצא בגבולות הלוח
                      if (ni >= 0 && ni < n && nj >= 0 && nj < n) {
                          const currentNeighbor = currentState[ni * n + nj];
                          // רק תאים שאינם זהב נכללים בממוצע
                          if (!currentNeighbor.isGold) {
                              totalBrightness += currentNeighbor.k; // הוספת ערך הבהירות (האינדקס) לסכום
                              neighborCount++; // ספירת המשתתפים בממוצע
                          }
                      }
                  }
              }
              ...
              if (neighborCount > 0) {
                  // חישוב הממוצע ועיגול למספר השלם הקרוב ביותר
                  const averageBrightness = totalBrightness / neighborCount;
                  nextStateK[i] = Math.round(averageBrightness);
              } else {
                  // במקרה הנדיר שתא מוקף כולו בתאי זהב, הוא שומר על צבעו
                  nextStateK[i] = cell.k;
              }
          }
      ...
      function getGeneticColor(parentHexColors) {
          ...
          const r = getRandomComponent('r');
          const g = getRandomComponent('g');
          const b = getRandomComponent('b');
