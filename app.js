(function() {
    ...
    function updateColorPickerButtonUI() {
      ...
      function prepareForLifeLongPress() {
          performAction(() => {
              // 1. Paint background with the darkest color
              const darkestIndex = 0; // Palettes are sorted, so 0 is darkest
              tiles().forEach(el => {
                  clearGoldOverlay(el);
                  setIndex(el, darkestIndex);
              });
              
              // 2. Select the lightest color
              const currentPalette = palette();
              const lightestIndex = currentPalette.length - 1; // Last color is lightest
              if (lightestIndex >= 0) {
                  selectedColor = currentPalette[lightestIndex];
                  selectedColorIndex = lightestIndex;
                  isRainbowModeActive = false; // Ensure rainbow mode is off
                  updateColorPickerButtonUI();
                  updateGlowEffect();
              }
          });
      ...
      function handlePointerDownCtrl(e) {
          ...
          longPressTimer = setTimeout(() => {
              ...
              // +++ MODIFIED: Special long-press for Game of Life button
